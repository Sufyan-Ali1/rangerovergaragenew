const DEFAULT_PORTAL_BASE_URL = "https://portal.voguetechnics.co.uk/api/v1";
...
export function getPortalConfig() {
  ...
  const websiteSlug =
    process.env.RRG_WEBSITE_SLUG ||
    process.env.WEBSITE_SLUG ||
    "rangerover-garage-new";
  const baseUrl = process.env.PORTAL_BASE_URL || DEFAULT_PORTAL_BASE_URL;
...
export function buildPortalVrmSearchUrl(vrm: string) {
  ...
  return `${baseUrl}/vrm-search?${params.toString()}`;
}
