...
function GetQuoteContent() {
    ...
    const fetchVehicle = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          // Check if API actually returned vehicle details
  ...
  useEffect(() => {
    if (isSuccess && countdown === 0) router.push("/");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Extract engine code from engine number
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6 pt-32">
        <motion.div
          ...
          className="max-w-xl w-full bg-white border border-gray-100 p-12 rounded-[3rem] text-center shadow-2xl"
        >
          <div className="w-20 h-20 bg-primary/10 rounded-full flex items-center justify-center mx-auto mb-8">
            <CheckCircle2 className="w-10 h-10 text-primary" />
          </div>
          <h2 className="text-2xl font-heading font-black text-slate-900 mb-4 uppercase tracking-tight leading-tight">
            Thank You! Your Quote Has Been{" "}
            <span className="text-primary italic">Received</span>
          </h2>
          <p className="text-gray-500 text-base mb-2 font-medium leading-relaxed">
            A member of our team will contact you shortly. For urgent enquiries, call us directly:
          </p>
          <a
            href="tel:07477733313"
            className="text-2xl font-heading font-black text-primary block mb-8 hover:underline italic tracking-widest"
          >
            07477 733313
          </a>
          <div className="bg-gray-50 py-3 px-6 rounded-full inline-flex items-center gap-3 mb-10 border border-gray-100">
            <Loader2 className="w-4 h-4 text-primary animate-spin" />
            <p className="text-gray-400 text-xs font-bold uppercase tracking-widest">
              Redirecting in{" "}
              <span className="text-primary">{countdown}</span> seconds...
            </p>
          </div>
          <br />
          <Link
